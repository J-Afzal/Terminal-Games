...
namespace TerminalGames
{
    ...
    bool Battleships::IsShipPresent(std::array<std::array<std::string, g_BATTLESHIPS_BOARD_WIDTH>, g_BATTLESHIPS_BOARD_HEIGHT> &board)
    {
        for (const std::array<std::string, g_BATTLESHIPS_BOARD_WIDTH> &currentRow : board)
        {
            ...
            {
                auto currentValueFindLocation = ImplementStdRangesFind(g_BATTLESHIPS_SHIP_NAMES.begin(), g_BATTLESHIPS_SHIP_NAMES.end(), currentValue);
        ...
        for (uint32_t currentShip = 0; currentShip < g_BATTLESHIPS_SHIP_COUNT; currentShip++)
        {
            while (true)
            {
                ...
                for (std::tuple<uint32_t, uint32_t> currentPosition : shipPositions)
                {
                    ...
                    auto currentPositionFindLocation = ImplementStdRangesFind(
        ...
        const std::tuple<uint32_t, uint32_t> &command)
    {
        ...
        auto currentValueFindLocation = ImplementStdRangesFind(g_BATTLESHIPS_SHIP_NAMES.begin(), g_BATTLESHIPS_SHIP_NAMES.end(), opponentBoard[row][column]); // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
